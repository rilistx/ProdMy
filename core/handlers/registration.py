...
from core.models.querys import search_user, get_language_one, get_currency_one, get_country_one, create_user, create_separator
...
async def start(message: Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    if user_exist:
        lang = await get_language_one(session=session, language_id=user_exist.language_id)

        await message.answer(
            text=connector[lang.abbreviation]['message']['start']['authorized'],
        )
