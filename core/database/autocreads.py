...
async def create_db() -> None:
    ...
    async with async_session_maker() as session:
        await create_language(session, connector_default_lang['language'])
        await create_currency(session, connector_default_lang['currency'])
        await create_catalog(session, connector_default_lang['catalog'])
        await create_subcatalog(session, connector_default_lang['catalog'])
        await create_country(session, connector_default_lang['country'])
        await create_region(session, connector_default_lang['country'])
        await create_city(session, connector_default_lang['country'])
...
async def create_admin(session: AsyncSession) -> None:
    user_exist = await search_user(session=session, user_id=admin['id'])

    if not user_exist:
        lang = await get_language_one(session=session, language_abbreviation=default_language)
