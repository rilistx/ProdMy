...
from core.utils.message import get_text_registration_start, get_text_registration_contact, \
...
async def start(
    ...
    text = await get_text_registration_start(
        lang=lang,
    )
...
async def contact(
    ...
    language = await get_language_one(
        session=session,
    ...
    success_test = await get_text_registration_contact(
        lang=lang,
    )
    support_text = await get_text_registration_support(
        lang=lang,
    )
    channel_text = await get_text_registration_channel(
        lang=lang,
    )

    reply_markup_success = ReplyKeyboardRemove()
    reply_markup_support = get_support_button(
        lang=lang,
    )
    reply_markup_channel = get_channel_button(
        lang=lang,
    )
